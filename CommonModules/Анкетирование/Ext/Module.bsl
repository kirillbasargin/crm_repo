...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		Если НЕ СерверныеОбработчики["СтандартныеПодсистемы.ВариантыОтчетов\ПриНастройкеВариантовОтчетов"] = Неопределено Тогда
...
Процедура ЗаполнитьДеревоШаблонаАнкеты(Форма, ИмяДереваАнкеты, ШаблонАнкеты) Экспорт
	...
	//<>, Басаргин (29.11.2018) {	
	Если ДеревоАнкетыСервер.Строки.Количество() Тогда
		ДеревоАнкетыСервер.Строки[0].Строки.Сортировать("КодРаздела");
	КонецЕсли;
	//<> }
...
Процедура ДобавитьСтрокиДереваАнкеты(Выборка,СтрокаРодитель,УровеньРекурсии,Форма)
	...
	Пока Выборка.Следующий() Цикл
		...
		//<>, Басаргин (29.11.2018) {
		Если НоваяСтрока.ТипСтроки = "Раздел" Тогда
			НоваяСтрока.КодРаздела = Выборка.Код;
		КонецЕсли;	
		//<> }
...
//
Процедура ПостроениеФормыЗаполненияПоРазделу(Форма,ТекущиеДанныеДеревоРазделов, ЭтоВебКлиент = Ложь) Экспорт //<607656>, Басаргин (20.01.2017)
	...
	Если ТекущиеДанныеДеревоРазделов.ТипСтроки = "Раздел" Тогда
		...
		Если ТекущийВариантИнтерфейсаКлиентскогоПриложения() = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2 Тогда
			...
			Форма.Элементы.НадписьВступление.Шрифт = Новый Шрифт(, 8, Истина, , , );  // 20
		Иначе
			Форма.Элементы.НадписьВступление.ЦветТекста = Новый Цвет(0, 150, 70);
			Форма.Элементы.НадписьВступление.Шрифт = Новый Шрифт(, ?(ЭтоВебКлиент, 22, 12), Ложь, , , ); // 22 
		КонецЕсли;
	...
	Иначе
		...
		Форма.Элементы.НадписьВступление.Шрифт = Новый Шрифт(, 8, Ложь, , , ); //22 
	...
	Если ТекущиеДанныеДеревоРазделов.ТипСтроки = "Раздел" Тогда
		...
		СформироватьЭлементыФормыДляРаздела(Форма, ЭтоВебКлиент);
	КонецЕсли;
...
Процедура СформироватьЭлементыФормыДляРаздела(Форма, ЭтоВебКлиент = Ложь) //<607656>, Басаргин (20.01.2017) 

	Для каждого СтрокаТаблицы Из Форма.ТаблицаВопросовРаздела Цикл
	
		ДобавитьЭлементыФормыПоСтрокеТаблицы(СтрокаТаблицы,Форма.Элементы.ГруппаТелоАнкеты,Форма, ЭтоВебКлиент);
...
//
Функция ДобавитьЭлементыФормыПоСтрокеТаблицы(СтрокаТаблицы,ЭлементГруппы,Форма, ЭтоВебКлиент = Ложь) //<607656>, Басаргин (20.01.2017)

	Если СтрокаТаблицы.ТипСтроки = "Раздел" Тогда
		Возврат ДобавитьЭлементыРаздел(СтрокаТаблицы,ЭлементГруппы,Форма, ЭтоВебКлиент);
	ИначеЕсли СтрокаТаблицы.ТипСтроки = "Вопрос" Тогда
		ДобавитьЭлементыВопрос(СтрокаТаблицы,ЭлементГруппы,Форма, ЭтоВебКлиент);
	КонецЕсли;
...
//   ГруппаФормы   - группа созданных элементов.
//
Функция ДобавитьЭлементыРаздел(СтрокаТаблицы,ЭлементГруппы,Форма, ЭтоВебКлиент = Ложь) //<607656>, Басаргин (20.01.2017)
	...
	////<>, Басаргин (26.01.2017) {
	//ЭлементРаздел.РастягиватьПоГоризонтали = Истина;
	//ЭлементРаздел.Ширина = 280;
	////<> }
...
//  Форма         - Управляемая форма- - форма для которой добавляются элементы.
//
Процедура ДобавитьЭлементыВопрос(СтрокаТаблицы,ЭлементГруппа,Форма, ЭтоВебКлиент = Ложь) //<607656>, Басаргин (20.01.2017) 
	...
	////<>, Басаргин (26.01.2017) {
	ЭлементГруппаВопроса.Ширина = 280; //ШиринаПодчиненныхЭлементовФормы.Одинаковая
	...
	//<>, Басаргин (26.01.2017) {
	ЭлементГруппаВопросКомментарий.РастягиватьПоГоризонтали = Истина;
	ЭлементГруппаВопросКомментарий.Ширина = 280;
	//<> }
	
	//<>, Басаргин (16.01.2017) {
	Если ЭтоВебКлиент Тогда
		Элемент.Шрифт = Новый Шрифт(, РазмерШрифтаВеб, Истина, , , );
	КонецЕсли;
	//<> }
	
	Если СтрокаТаблицы.ТипВопроса = Перечисления.ТипыВопросовШаблонаАнкеты.Табличный Тогда
		
		ДобавитьЭлементыТабличныйВопрос(СтрокаТаблицы,ЭлементГруппаВопроса,Форма, ЭтоВебКлиент);
	...
	Иначе
		...
		ИначеЕсли СтрокаТаблицы.ТипОтвета = Перечисления.ТипыОтветовНаВопрос.Булево Тогда
			...
			//<>, Басаргин (16.01.2017) {
			Если ЭтоВебКлиент Тогда
				Элемент.ШрифтЗаголовка = Новый Шрифт(, РазмерШрифтаВеб, Истина, , , );
			КонецЕсли;	
			//<> }
...
//
Процедура ДобавитьЭлементыТабличныйВопрос(СтрокаТаблицы,ЭлементГруппа,Форма, ЭтоВебКлиент = Ложь) //<607656>, Басаргин (20.01.2017)
	...
	//<>, Басаргин (16.01.2017) {
	Если ЭтоВебКлиент Тогда
		ЭлементТаблица.Шрифт = Новый Шрифт(, РазмерШрифтаВеб, Истина, , , );
	КонецЕсли;	
	//<> }
...
Процедура ЗаполнитьДеревоРазделов(Форма,ДеревоРазделов) Экспорт
	...
	//<>, Басаргин (29.11.2018) {
	ДеревоАнкетыСервер = ДанныеФормыВЗначение(ДеревоРазделов, Тип("ДеревоЗначений")); //Форма.РеквизитФормыВЗначение(ИмяДереваАнкеты);
	Если ДеревоАнкетыСервер.Строки.Количество() Тогда
		ДеревоАнкетыСервер.Строки.Сортировать("КодРаздела");
	КонецЕсли;
	//Форма.ЗначениеВРеквизитФормы(ДеревоАнкетыСервер,ИмяДереваАнкеты);
	ЗначениеВДанныеФормы(ДеревоАнкетыСервер, ДеревоРазделов);
	//<> }
...
Функция ПолучитьВыборкуРазделовПоШаблонуАнкеты(ШаблонАнкеты)
	...
	//<> }
	|			(НЕ ВопросыШаблонаАнкеты.ЭтоГруппа)
	|		
	|		СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьСтрокиВДеревоРазделов(ВыборкаРазделы,Родитель)
	...
	Пока ВыборкаРазделы.Следующий() Цикл
		...
		//<>, Басаргин (29.11.2018) {
		НовыйЭлементДерева.КодРаздела = ВыборкаРазделы.Код;
		//<> }
...
Процедура ПолучитьИнформациюПоВопросамАнкеты(Форма, ШаблонАнкеты, Раздел,ПолныйКодРаздела)
	...
	//|				ВопросыШаблонаАнкеты.Владелец = &ШаблонАнкеты И 
	//<> }	
	|				ВопросыШаблонаАнкеты.Родитель = &Раздел
	|		
	|			ОБЪЕДИНИТЬ ВСЕ
	|		
	|			ВЫБРАТЬ
	...
	|			ГДЕ
	//<>, Басаргин (16.01.2017) {
	//|				ВопросыШаблонаАнкетыСоставТабличногоВопроса.Ссылка.Владелец = &ШаблонАнкеты И 	
	//<> }
	|				ВопросыШаблонаАнкетыСоставТабличногоВопроса.Ссылка.Родитель = &Раздел)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
