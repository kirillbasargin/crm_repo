...
Процедура ПолучениеЗаявокСПочты() Экспорт
	...
	Для Каждого ЭлементВходящееСообщение Из ТаблицаВходящихСообщений Цикл				
		...
		//Если НЕ ЭлементВходящееСообщение.ИдентификаторСообщения = "E1gP4gq-0005zU-Sa@p504923.vps" Тогда
		//	Продолжить;
		//КонецЕсли;
		
		Если НЕ (НачалоДня(ЭлементВходящееСообщение.ДатаОтправления) = НачалоДня(ТекущаяДата())
					ИЛИ НачалоДня(ЭлементВходящееСообщение.ДатаОтправления) = НачалоДня(ТекущаяДата()) - 3600 * 24) Тогда
			//ИЛИ ЗаявкаПоПисьмуСоздана(ЭлементВходящееСообщение.ИдентификаторСообщения) Тогда
			Продолжить;
		КонецЕсли; 		
		
		Если ЗаявкаПоПисьмуСоздана(ЭлементВходящееСообщение.ИдентификаторСообщения) Тогда
			Продолжить;
		КонецЕсли;
		...
		//<900185>, Басаргин (20.11.2018) {
		ИначеЕсли СтрНайти(ЭлементВходящееСообщение.Отправитель, "no-reply-smsforwarder@cofp.ru") > 0 Тогда  		
			ЗаявкаОбъект.ТипВызова = Перечисления.ТипыВызовов.ЗаявкаДомКлик;
		//<900185> }
		Иначе
		...
		//<900185>, Басаргин (20.11.2018) {
		Если ЗаявкаОбъект.ТипВызова = Перечисления.ТипыВызовов.ЗаявкаДомКлик Тогда
			ЗаявкаОбъект.Сайт = Неопределено;
			ЗаявкаОбъект.Проект = Неопределено;
			ЗаявкаОбъект.ВидНедвижимости = Неопределено;					
			
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	СоответствиеПроектовДомКлик.Проект КАК Проект,
			|	СоответствиеПроектовДомКлик.ВидНедвижимости КАК ВидНедвижимости,
			|	СоответствиеПроектовДомКлик.Сайт КАК Сайт,
			|	СоответствиеПроектовДомКлик.НазваниеПроектаДомКлик КАК НазваниеПроектаДомКлик
			|ИЗ
			|	РегистрСведений.СоответствиеПроектовДомКлик КАК СоответствиеПроектовДомКлик";
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					Если СтрНайти(ЗаявкаОбъект.reqText, ВыборкаДетальныеЗаписи.НазваниеПроектаДомКлик) Тогда
						ЗаявкаОбъект.Сайт = ВыборкаДетальныеЗаписи.Сайт;
						ЗаявкаОбъект.Проект = ВыборкаДетальныеЗаписи.Проект;
						ЗаявкаОбъект.ВидНедвижимости = ВыборкаДетальныеЗаписи.ВидНедвижимости;							
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;					
		КонецЕсли;
...
Процедура СозданиеЗаявокНаЗвонокИзВзаимодействий() Экспорт
	...
	|	ПредметыПапкиВзаимодействий.Взаимодействие.Описание КАК Описание,
	...
	|	ВТ_Взаимодействия.Описание КАК Описание,
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл			
			...
			ЗаявкаОбъект.reqText = Выборка.Описание;
...
Процедура ЗагрузкаЗаявокДомКлик() Экспорт
	...
	// отправка запроса
	ОтветСервера = ОтправкаSMSЧерезInfobip.ВыполнитьЗапрос("inbox/reports", Логин, Пароль, ПараметрыЗапроса, "GET"); 
	Если НЕ ЗначениеЗаполнено(ОтветСервера) Тогда
		ОписаниеОшибки = НСтр("ru = 'Не удалось получить сообщения от ДомКлик'");		
	...
	Для каждого Результат Из ОтветСервера["results"] Цикл		
		Если НЕ Результат.Свойство("keyword") ИЛИ НЕ ВРег(Результат["keyword"]) = "КЛИЕНТ ДОМКЛИК" Тогда
			Продолжить;
		КонецЕсли;
			
		ЗаявкаОбъект = Документы.ЗаявкаНаЗвонок.СоздатьДокумент();
		ЗаявкаОбъект.id = Строка(Новый УникальныйИдентификатор);
		ЗаявкаОбъект.reqID = Результат["messageId"];
		
		ТекущаяДата = ТекущаяДата();
		ЗаявкаОбъект.Дата = ТекущаяДата;	
		ЗаявкаОбъект.ДатаПоследнейЗагрузки = ТекущаяДата;		

		ЗаявкаОбъект.reqText = Результат["text"];
		ЗаявкаОбъект.Тема = "Заявка от ДомКлик(загружена через Infobip)";
		
		СтрокаДаты = Результат["receivedAt"];			
		ЧастиДатыМассив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаДаты, ".");
		Если ЧастиДатыМассив.Количество() Тогда
			СтрокаДаты = ЧастиДатыМассив[0];		
		КонецЕсли;
		
		ДиВ = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаДаты, "T");			
		Если ДиВ.Количество() Тогда			
			...
			ЗаявкаОбъект.reqData = Дата(стрДата + " " + стрВ);
		КонецЕсли;
	...
	Если Константы.ИспользоватьАвтораспределениеЗаявок.Получить() Тогда 
		АвтоРаспределениеЗаявок(Перечисления.ГруппыРаспределенияЗаявок.ГПТ, Перечисления.ВидыЗаявокНаЗвонок.ЗаказЗвонка, Перечисления.ТипыВызовов.ЗаявкаДомКлик);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявкиНаCallBack(ПараметрыСайта, НайденыеКлиенты_МВТ = Неопределено, СписокОС = Неопределено)
	...
	Попытка
		...
		ПараметрыПОСТ = "login=" + ПараметрыСайта.Login + "&password=" + ПараметрыСайта.Password + "&action=getFeedbacksJSON"; // ПараметрыСайта.Password
		
		//КодироватьСтроку(ПараметрыСайта.Password, СпособКодированияСтроки.КодировкаURL)
...
Функция ПолучитьНомерИзТекста(Текст)
    ...
    RegExp.Pattern = "((8|\+7|9)[\- ]?)+(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}[^руб]"; //((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}
