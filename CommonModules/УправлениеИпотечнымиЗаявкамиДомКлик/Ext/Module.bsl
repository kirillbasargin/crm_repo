...
Функция СозданиеДокументов(ПараметрыAPI, ID_Анкеты, Документ, ТипДокумента, ПодтипДокумента = Неопределено, Ошибки = "", Тест = Ложь) Экспорт
	...
	Если НЕ ОтветСервера = Неопределено Тогда
		Если ТипЗнч(ОтветСервера) = Тип("Структура") И ОтветСервера.Свойство("code") Тогда
			...
			КонецЕсли;			
		Иначе							
			СтруктураВозврата = Новый Структура("id, path, create_date, name, ПодтипДокумента, СтатусДокумента, Отправлять");
...
Функция ОбновитьДокумент(ПараметрыAPI, ID_Анкеты, ID_Документа, ТипДокумента, ПодтипДокумента = Неопределено, Ошибки = "", Тест = Ложь) Экспорт
	...
	ТелоЗапроса.Вставить("documentTypeId", СтрЗаменить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТипДокумента, "ID"), Символы.НПП, ""));
	Если ЗначениеЗаполнено(ПодтипДокумента) Тогда
		ТелоЗапроса.Вставить("documentSubtypeId", СтрЗаменить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПодтипДокумента, "ID"), Символы.НПП, ""));	
	КонецЕсли;	
...
Функция ОтправитьСделку(ПараметрыAPI, ID_Заявки, Ошибки = "", Тест = Ложь) Экспорт
	...
	Если НЕ ОтветСервера = Неопределено Тогда	
		КодСостояния = ОтветСервера;	
	Иначе
		Ошибки = Ошибки + Символы.ПС + "ОтправитьСделку: Не удалось получить результат запроса";		
	КонецЕсли;
...
Функция ПолучитьIDСтатусаЗаявки(Статус) Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СтатусыИпотечныхЗаявок.ID КАК ID
	|ИЗ
	|	Справочник.СтатусыИпотечныхЗаявок КАК СтатусыИпотечныхЗаявок
	|ГДЕ
	|	СтатусыИпотечныхЗаявок.Ссылка = &Ссылка";
...
Функция ПолучитьОбновляемыеЗначенияСделки(Данные, ИпотечнаяЗаявка, ТекущийСтатусЗаявки, КомментарийСтатусаЗаявки) Экспорт //, ТолькоОтправкаПисем = Ложь
	...
	Если СтруктураСделки.Свойство("СтатусЗаявки") Тогда
		...
		Иначе
			...
			СтруктураСделки.Вставить("Черновик", Ложь);
		КонецЕсли;
		
		Если НЕ ТекущийСтатусЗаявки = СтатусЗаявки Тогда
			СтруктураСделки.Вставить("ДатаУстановкиСтатуса", ТекущаяДата());
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатусЗаявки, "ID") = 8070 Тогда
				СтруктураСделки.Вставить("ДатаПодачиЗаявки", ТекущаяДата());
			КонецЕсли;
...
Функция ПолучитьФайлВБинарномВиде(ИмяФайла, ТипДокумента, ПодтипДокумента = Неопределено, ДвоичныеДанные, boundary, Расширение)
    ...
    ЗаписьДанных = Новый ЗаписьДанных(Поток, КодировкаТекста.ANSI); 
...
Функция ПреобразоватьКДате(ДатаСтрокой) Экспорт
	...
	Возврат МестноеВремя(ДатаРезультат);
...
Функция СоздатьСтруктуруСделкиДляОтправки(ИпотечнаяЗаявка) 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ИпотечнаяЗаявка.agentPhone КАК agentPhone,
...
Функция ПолучитьНомерТелефонаАгента(Пользователь = Неопределено) Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		НомерТелефона = ВыборкаДетальныеЗаписи.Представление;		
	КонецЕсли;
