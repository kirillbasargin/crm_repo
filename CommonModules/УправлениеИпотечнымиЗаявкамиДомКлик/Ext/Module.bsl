...
Функция СозданиеАнкеты(ПараметрыAPI, ИпотечнаяЗаявка, Ошибки = "", Тест = Ложь) Экспорт
...
Функция СоздатьСтруктуруАнкеты(ПараметрыAPI, АнкетаСсылка) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда		
		...
		СтруктураПараметров = ПолучитьКонтактныеДанныеКлиента(АнкетаСсылка.КонтактноеЛицо);	
