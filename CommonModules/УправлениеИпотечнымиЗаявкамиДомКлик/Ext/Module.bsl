...
Функция СозданиеАнкеты(ПараметрыAPI, ID_Заявки, Анкета, ТипАнкеты, Ошибки = "", Тест = Ложь) Экспорт
	...
	ТелоЗапроса = СоздатьСтруктуруАнкеты(ПараметрыAPI, Анкета, ТипАнкеты); //СоздатьСтруктуруСделкиДляОтправки(ИпотечнаяЗаявка);
	...
	Если НЕ ОтветСервера = Неопределено Тогда
		...
		Иначе	
			//ИдентификаторСделки = ОтветСервера.id;
			//"acceptance": {
			//"applicationId": 515783,
			//"lastRequestMoment": null,
			//"signMoment": null,
			//"status": "NOT_SENT"
			//},
			...
			//СтруктураВозврата.ApplicationID = ;
			//СтруктураВозврата.Статус = ;
			//СтруктураВозврата.ДатаПоследнегоИзменения = ;
			//СтруктураВозврата.ДатаПодтверждения = "";
			//СтруктураВозврата.ТипАнкеты = "";
...
Функция ПреобразоватьСтруктуруАнкетыВJSON(СтруктураАнкеты) Экспорт
	...
	Если НЕ СтруктураАнкеты = Неопределено Тогда
		Попытка
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.УстановитьСтроку();
			ЗаписатьJSON(ЗаписьJSON, СтруктураАнкеты);			
			СтруктураАнкетыJSON = ЗаписьJSON.Закрыть();
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошбка преобразования структуры анкеты в JSON'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , , "Ошибка:" + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;	
		
	Возврат СтруктураАнкетыJSON;	
