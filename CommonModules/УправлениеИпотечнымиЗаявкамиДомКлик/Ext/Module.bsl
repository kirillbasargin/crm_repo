...
Функция СозданиеСделки(ПараметрыAPI, ИпотечнаяЗаявка, Ошибки = "", Тест = Ложь) Экспорт
	...
	Если НЕ ОтветСервера = Неопределено Тогда
		Если ТипЗнч(ОтветСервера) = Тип("Структура") И ОтветСервера.Свойство("code") Тогда
			Ошибки = "Ошибка при отправке заявки." + "Код " + ОтветСервера.code + Символы.ПС;
...
Функция СозданиеДокументов(ПараметрыAPI, ID_Анкеты, Документ, ТипДокумента, ПодтипДокумента = Неопределено, Ошибки = "", Тест = Ложь) Экспорт
	...
	ПараметрыЗапроса = ПолучитьПараметрыЗапроса(HTTPМетод, Сервер, АдресРесурса, ТелоЗапроса, Прокси, , , , ФайлЗапроса);
	
	ОтветСервера = ВыполнитьHTTPЗапрос(ПараметрыЗапроса, ЗаголовокHTTP);	
	Если НЕ ОтветСервера = Неопределено Тогда
		Если ТипЗнч(ОтветСервера) = Тип("Структура") И ОтветСервера.Свойство("code") И ОтветСервера.code = 0 Тогда
			Ошибки = "Ошибка при отправке заявки.";
			Если ОтветСервера.Свойство("internalMessage") Тогда
				Ошибки = Ошибки + ОтветСервера.internalMessage + Символы.ПС;
			КонецЕсли;
			Если ОтветСервера.Свойство("message") Тогда
		...
		Иначе	
			...
			//"id": 375481,
			//"createdTime": "2018-07-25T16:03:14.520Z",
			//"fileId": 1732649,
			//"dealId": 494343,
			//"applicationId": 479043,
			//"userId": 20639,
			//"documentTypeId": 20090,
			//"documentSubtypeId": 23090,
			//"extension": null,
			//"path": "https://qa-ipoteka.domclick.ru/fs/api/v2/files/1732649",
			//"filename": ".pdf",
			//"accepted": null,
			//"comment": null,
			//"parentId": null		
...
Функция ВыполнитьHTTPЗапрос(ПараметрыЗапроса, Заголовки, ТолькоКодСостояния = Ложь)
	...
	HTTPЗапрос = ПодготовитьHTTPЗапрос(ПараметрыЗапроса.АдресРесурса, Заголовки, ПараметрыЗапроса.ТелоЗапроса, ПараметрыЗапроса.HTTPМетод, ПараметрыЗапроса.ИмяФайлаЗапроса); 
...
Функция ПодготовитьHTTPЗапрос(АдресРесурса, Заголовки, ПараметрыЗапроса, HTTPМетод = "POST", ИмяФайлаЗапроса = "") Экспорт
	...
	Иначе		
		Если HTTPМетод = "POST" Тогда
			Если НЕ ПустаяСтрока(ИмяФайлаЗапроса) Тогда
...
Функция ПолучитьПараметрыЗапроса(HTTPМетод, Сервер, АдресРесурса, ТелоЗапроса, Прокси, Порт = Неопределено, Пользователь = Неопределено, Пароль = Неопределено, ИмяФайлаЗапроса = "") 
...
Функция ПоместитьФайлВТелоЗапроса(ПрикрепляемыйФайл, Boundary, ТипДокумента, ПодтипДокумента, Ошибки = "") 
	...
	ЗаписьТекста = Новый ЗаписьТекста(имяФайлаСПараметрами, КодировкаТекста.UTF8); //Символы.ПС, Истина, Символы.ПС); 
	ЗаписьТекста.ЗаписатьСтроку("--"+boundary);    
	ЗаписьТекста.ЗаписатьСтроку("Content-Disposition: form-data; name=""file""; filename=""" + ИмяФайлаДляСервера + """");
	ЗаписьТекста.ЗаписатьСтроку("Content-Type:");
	...
	//ФайлОтправкиНачало.ЗаписатьСтроку("Content-Disposition: form-data; name=""file""; filename=""" + ИмяФайлаДляСервера + """"); 
	//	
	...
	//
	////
	////ФайлОтправкиКонец.ЗаписатьСтроку("Content-Disposition: form-data; name=""typeId"");
	////ФайлОтправкиКонец.ЗаписатьСтроку("typeId=" + ТипДокумента);
	////ФайлОтправкиКонец.ЗаписатьСтроку("Content-Disposition: form-data; name=""typeId"");
	////ФайлОтправкиКонец.ЗаписатьСтроку("subtypeId=" + ПодтипДокумента);
	//
	//ТекстФайлконец = //"--" + Boundary + "
	//"Content-Disposition: form-data; name=""typeId""
	//|
	//|" + ТипДокумента;
	//
	//ФайлОтправкиКонец.ЗаписатьСтроку(ТекстФайлконец);

	//Если ЗначениеЗаполнено(ПодтипДокумента) Тогда
	//	ТекстФайлконец = "--" + Boundary + "
	//	|Content-Disposition: form-data; name=""subtypeId""
	//	|
	//	|" + ПодтипДокумента;
	//	ФайлОтправкиКонец.ЗаписатьСтроку(ТекстФайлконец);	
	//КонецЕсли;
...
Функция СоздатьСтруктуруАнкеты(ПараметрыAPI, АнкетаСсылка, ТипАнкеты) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда		
		...
		СтруктураПараметров = ПолучитьКонтактныеДанныеКлиента(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(АнкетаСсылка, "КонтактноеЛицо"));	
...
Функция ПолучитьСтруктуруАнкеты(ПараметрыAPI, РезультатЗаполнения, ВыгрузкаСтруктурыВопросов, ВыгрузкаСтруктурыОтветов = Неопределено, ЗапрашиватьСтуктуруАдресаDaData = Ложь, ВыгрузкаСтруктурыВопросов_АдресРегистрации = Неопределено, ВыгрузкаСтруктурыВопросов_ФактическийАдрес = Неопределено)
	...
	Если ЗапрашиватьСтуктуруАдресаDaData Тогда		
		Если НЕ ВыгрузкаСтруктурыВопросов_АдресРегистрации = Неопределено Тогда
			...
			СтруктураАнкеты.Вставить("registrationAddress", НормализоватьАдресDaData(ПараметрыAPI, АдресРегистрации, "АдресРегистрацииДомКлик"));
		Иначе 
		...
		Если НЕ ВыгрузкаСтруктурыВопросов_ФактическийАдрес = Неопределено Тогда
			...
			СтруктураАнкеты.Вставить("actualAddress", НормализоватьАдресDaData(ПараметрыAPI, ФактическийАдрес, "ФактическийАдресДомКлик"));	
		Иначе
