...
Процедура УстановитьПометкуУдаленияУВложений(Ссылка, ПометкаУдаления) Экспорт
	...
	|ГДЕ
	|	НЕ Файлы.ПометкаУдаления И Файлы.ПометкаУдаления <> &ПометкаУдаления
...
Процедура ЗаполнитьПоДаннымАнкеты(ДанныеЗаполнения)
	...
	РасчетИпотеки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "РасчетИпотеки"); 
	НоваяСтрока = Анкеты.Добавить();
	НоваяСтрока.Анкета = ДанныеЗаполнения;
	//<897513> }
