...
Процедура СозданиеАнкетыНаСервере()
	...
	Для каждого Строка Из Объект.Анкеты Цикл
		...
		КонецЕсли;
		Ошибки = "";
		ID_Анкеты = УправлениеИпотечнымиЗаявкамиДомКлик.СозданиеАнкеты(Объект.ПараметрыПодключения, Объект.ID_Заявки, Строка.Анкета, Строка.ТипАнкеты, Ошибки, Объект.ТестоваяЗаявка);	
	КонецЦикла;
