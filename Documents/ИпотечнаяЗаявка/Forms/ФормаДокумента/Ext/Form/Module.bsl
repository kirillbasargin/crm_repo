...
Процедура УправлениеВидимостьюДоступностью()
	...
	Элементы.ГруппаКомментарии.Видимость = НЕ ДомКлик;
...
Процедура ОтправитьСделкуНаСервере()
	...
	Статус = Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("Title", "SENT_TO_BANK");
	Ошибки = "";
	КодВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.События(Объект.ПараметрыПодключения, Объект.ID_Заявки, Статус, Ошибки, Объект.ТестоваяЗаявка);
...
Процедура ПолучитьДанныеСделки_Асинхр(Результат, ДополнительныеПараметры) Экспорт	
	...
	//УстановитьОбновляемыеЗначенияСделки(Данные);
	
	ПолучитьОбновляемыеЗначенияСделки(ДанныеЗаявки, Объект.СтатусЗаявки);
...
&НаСервере
Функция ПолучитьОбновляемыеЗначенияСделки(Данные, СтатусЗаявки)
	
	ДанныеСделки = УправлениеИпотечнымиЗаявкамиДомКлик.ПреобразоватьОтветВСтруктуру(Данные);
	
	СтруктураВозврата = Новый Структура("СтатусЗаявки, УспешнаяОтправка, ДатаУстановкиСтатуса, СостояниеЗаявки, РешениеБанка, ДатаРешенияБанка");	
	
	//"events": [
	//{
	//"type": "CREDIT_SENT",
	//"createdTime": "2018-07-24T14:59:01.112Z",
	//"comment": null
	//}
	//]	
	...
	//Id заявки	id
	//Срок кредита (мес.)	loanPeriod
	//Запрошенная сумма кредита 	requestedSum
	//Одобренная сумма кредита	approvedSum
	//Ежемесячный платеж	monthlyPayment
	//Процентная ставка	interestRate
	//Переплата	overpaymentSum
	//Необходимый доход	requiredIncomeSum
	//
	//Предполагаемая стоимость ОН	sumSupposes
	//Предполагаемая стоимость ОН - Сумма кредита 	initialFee
	//Вид объекта недвижимости	productId
	//Отделение банка	officeId
	//Id пользователя	authorId
	//Статус заявки	dealStatusId
	//Номер телефона	agentPhone
	//ID основного заемщика	borrowerCasId
	//Документы «Анкета СБ»	applications
	//ТЧ «События по заявке»	events
	//Комментарий	comment
		
	//Если Данные.Свойство("dealStatusId") Тогда
	//	
	//	СтатусЗаявки = УправлениеИпотечнымиЗаявкамиДомКлик.ПолучитьСтатусЗаявкиПоID(Данные.dealStatusId);
	//	
	//	//Статус = Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("Title", Данные.dealStatusId);
	//	
	//	И НЕ СтатусЗаявки.Код = Данные.status 
	//	
	//	
	//	Объект.СтатусЗаявки = Справочники.СтатусыИпотечныхЗаявок.НайтиПоКоду(Данные.status);
	//	Если //Объект.СтатусЗаявки = ПредопределенноеЗначение("Справочник.СтатусыИпотечныхЗаявок.ВозвратНаДоработку") ИЛИ 
...
Процедура ОбновлениеАнкеты(Команда)
	...
	СтруктураВозврата = ОбновлениеАнкетыНаСервере(ТекущиеДанные.ApplicationID, ТекущиеДанные.Анкета, ТекущиеДанные.ТипАнкеты);
...
Процедура ОтправитьДокументы(Команда)
	...
	Для каждого Строка Из Объект.ПрикрепленныеФайлы Цикл
		Если НЕ Строка.Отправлять ИЛИ НЕ ЗначениеЗаполнено(Строка.ApplicationID) ИЛИ НЕ ЗначениеЗаполнено(Строка.ТипДокумента) Тогда
			Продолжить;	
		КонецЕсли;
