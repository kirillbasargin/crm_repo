...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.Свойство("ПрикрепленныеФайлы") Тогда
			Для каждого Файл Из Параметры.ПрикрепленныеФайлы Цикл
		...
		Объект.ИпотечныйАгент = Справочники.ИпотечныеАгенты.НайтиПоРеквизиту("Пользователь", Пользователи.ТекущийПользователь());
		//<897513> }		
	КонецЕсли;		
...
Процедура УправлениеВидимостьюДоступностью()
	...
	Элементы.СделкаДомКлик.Видимость = ДомКлик;
...
&НаСервере
Процедура ОбновитьТаблицуФайлов(Анкета = Неопределено, ApplicationID = 0)
...
Процедура ПослеВыбораАнкеты(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ОбновитьТаблицуФайлов(ВыбранныйЭлемент.Значение, ApplicationID);	
	
КонецПроцедуры

&НаСервере
Процедура АнкетыАнкетаПриИзмененииНаСервере()
...
Процедура АнкетыАнкетаПриИзменении(Элемент)
	АнкетыАнкетаПриИзмененииНаСервере();
КонецПроцедуры
...
&НаСервере
Процедура ОтправитьЗаявкуДомКлик()
	
	ЗаявкаУспешноОтправлена = Ложь;
	
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СтатусЗаявки, "ID") = 8060 Тогда		
		...
		//ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Заявка успешно отправлена.");
		Записать(Новый Структура("РежимЗаписи", РежимЗаписиДокумента.Проведение));
		ЗаявкаУспешноОтправлена = Истина;
		
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СтатусЗаявки, "ID") = 8090 Тогда		
		Ошибки = "";
		...
		Если Ошибки = "" И ID_Заявки = Объект.ID_Заявки Тогда
			Статус = Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("ID", 8070); //Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("Title", "SENT_TO_BANK");
			Ошибки = "";
		...
		КонецЕсли;				
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Отправка в текущем состояни невозможна");		
	КонецЕсли;
	
	Если ЗаявкаУспешноОтправлена Тогда
			...
			Если Ошибки = "" Тогда 
				Если НЕ СтруктураВозврата = Неопределено Тогда		
					...
					Для каждого Строка Из НайденныеСтроки Цикл
						Строка.ApplicationID = Строка.ApplicationID;	
					КонецЦикла;
				КонецЕсли;
			Иначе
...
&НаСервере
Процедура ОтправитьСделкуНаСервере()
	...
	Если Ошибки = "" Тогда
		...
		КонецЕсли;			
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка при синхронизации данных заявки перед отправкой: " + Ошибки);
		Возврат;
	КонецЕсли;

	ID_Статуса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СтатусЗаявки, "ID");
	Если ID_Статуса = 8060 ИЛИ ID_Статуса = 8090 Тогда
		АнкетыОдобрены = Истина;		
		Для каждого СтрокаАнкета Из Объект.Анкеты Цикл
			Если НЕ СтрокаАнкета.Статус = Справочники.СтатусыИпотечныхАнкетДомКлик.Одобрено Тогда
				АнкетыОдобрены = Ложь;
		...
		Если Ошибки = "" И ID_Заявки = Объект.ID_Заявки Тогда
			...
			Если Ошибки = "" И КодВозврата = 200 Тогда
				...
				Если Ошибки = "" Тогда
					...
					КонецЕсли;			
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка при синхронизации данных заявки после отправки: " + Ошибки);
					Возврат;
				КонецЕсли;
			Иначе
			...
			КонецЕсли;				
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка при отправке изменений по заявке перед перед отправкой: " + Ошибки);	
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Отправка в текущем состояни невозможна.");		
	КонецЕсли;			
	...
	//	КонецЕсли;
	//	
	//	//Если Объект.СтатусЗаявки = Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("ID", 8070) Тогда
	//		Ошибки = "";
	//		ID_Заявки = УправлениеИпотечнымиЗаявкамиДомКлик.ОбновлениеСделки(Объект.ПараметрыПодключения, Объект.Ссылка, Объект.ID_Заявки, Ошибки, Объект.ТестоваяЗаявка);			
	//		Если Ошибки = "" И ID_Заявки = Объект.ID_Заявки Тогда
	//			Статус = Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("ID", 8070); //Справочники.СтатусыИпотечныхЗаявок.НайтиПоРеквизиту("Title", "SENT_TO_BANK");
	...
	//		КонецЕсли;				
	//	Иначе
	//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка при повторной отправке заявки: " + Ошибки);	
	//	КонецЕсли;				
	//Иначе
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Отправка в текущем состояни невозможна.");		
	//КонецЕсли;
...
Процедура УстановитьОбновляемыеЗначенияСделкиНаФорме(ДанныеЗаявки)
	...
	Если СтруктураОбновляемыхЗначений.Свойство("АнкетыТЧ") Тогда
		...
		//	КонецЕсли;
		//КонецЦикла;
		//Если АнкетыОдобрены И Объект.СтатусЗаявки = УправлениеИпотечнымиЗаявкамиДомКлик.ПолучитьСтатусЗаявкиПоID(8060) Тогда
		//	Объект.ДатаПодачиЗаявки = ТекущаяДата();
		//	Объект.Черновик = Ложь;
		//	Объект.ДатаУстановкиСтатуса = ТекущаяДата();
		//	Объект.СтатусЗаявки = ПредопределенноеЗначение("Справочник.СтатусыИпотечныхЗаявок.ЗаявкаНаправленаВБанк");
...
Процедура СозданиеАнкеты(Команда)
	...
	СозданиеАнкетыНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура СозданиеАнкетыНаСервере()
...
&НаСервере
Функция ОбновлениеАнкетыНаСервере(ApplicationID, Анкета, ТипАнкеты)
		
	//Ошибки = "";
	//СтруктураВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.ОбновлениеАнкеты(Объект.ПараметрыПодключения, Объект.ID_Заявки, ApplicationID, Анкета, ТипАнкеты, Ошибки, Объект.ТестоваяЗаявка);
	//	
	//Если НЕ Ошибки = "" Тогда 
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Ошибки);
	//КонецЕсли;
...
Процедура ПослеВводаКодаПодтверждения(КодПодтверждения, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КодПодтверждения = Неопределено Тогда		
		...
		Если Ошибки = "" Тогда 
			...
			КонецЕсли;
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка при подписании согласия кодом: " + Ошибки);
		КонецЕсли;	
	КонецЕсли;
...
Процедура УдалениеДокументов(Команда)
	...
	Ошибки = "";
	КодВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.УдалениеДокументов(Объект.ПараметрыПодключения, ТекущиеДанные.ApplicationID, ТекущиеДанные.id, Ошибки, Объект.ТестоваяЗаявка);
...
Процедура ОбновитьДокумент(Команда)
	...
	Ошибки = "";
	СтруктураВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.ОбновитьДокумент(Объект.ПараметрыПодключения, ТекущиеДанные.ApplicationID, ТекущиеДанные.id, ТекущиеДанные.ТипДокумента, ТекущиеДанные.ПодтипДокумента, Ошибки, Объект.ТестоваяЗаявка);	
	Если Ошибки = "" Тогда 
		Если НЕ СтруктураВозврата = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Документ обновлен.");
			СтрокаТЧ = Объект.ПрикрепленныеФайлы.НайтиПоИдентификатору(ТекущиеДанные.ПолучитьИдентификатор());
