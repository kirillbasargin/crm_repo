...
Процедура ОтправитьСделкуНаСервере()
	...
	//сделать проверку статуса. Если старый статус НЕ = обновленному, сообщить что данные были синхронизированны. укажите коммент и Отправьте еще раз.
...
&НаСервере
Процедура ОтправитьДокументыНаСервере()
...
Функция УдалениеДокументовНаСервере(ApplicationID, id, Ошибки)
	...
	ИдентификаторыДокументовДляУдаления = Новый Массив;
		...
		Если Анкета.ID = ApplicationID Тогда
			Для каждого Документ Из Анкета.documents Цикл
				Если Документ.id = id Тогда
					Если ИдентификаторыДокументовДляУдаления.Найти(Документ.id) = Неопределено Тогда
						ИдентификаторыДокументовДляУдаления.Добавить(Документ.id);
					КонецЕсли;
	...
	Если НЕ ИдентификаторыДокументовДляУдаления.Количество() Тогда
		Ошибки = Ошибки + "Идентификаторы документов не найдены.";
		Возврат 0;
	КонецЕсли;
		
	Для каждого ID_Документа Из ИдентификаторыДокументовДляУдаления Цикл		
		Ошибки = "";
...
&НаСервере
Процедура СоздатьЧерновикСделкиНаСервере()
