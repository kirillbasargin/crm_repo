...
&НаСервере
Процедура ОбновлениеСделкиНаСервере()
...
&НаСервере
Процедура ОбновлениеАнкетыНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ОбновлениеАнкеты(Команда)
	ОбновлениеАнкетыНаСервере();
КонецПроцедуры

&НаСервере
Процедура УдалениеАнкетыСозаемщикаНаСервере()
КонецПроцедуры

&НаКлиенте
Процедура УдалениеАнкетыСозаемщика(Команда)
	
	//УдалениеАнкетыСозаемщикаНаСервере();
	...
	ID_Анкеты = ТекущиеДанные.ApplicationID;
	Ошибки = "";
	РезультатУдаления = УправлениеИпотечнымиЗаявкамиДомКлик.УдалениеАнкетыСозаемщика(Объект.ПараметрыПодключения, Объект.ID_Заявки, ID_Анкеты, Ошибки, Объект.ТестоваяЗаявка);
	
	Если Ошибки = "" Тогда 
		Если РезультатУдаления = 200 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Анкета успешно удалена.");
		КонецЕсли;
...
&НаСервере
Процедура СозданиеДокументовНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура СозданиеДокументов(Команда)
	СозданиеДокументовНаСервере();
КонецПроцедуры


&НаСервере
Процедура ОтправитьСМСКлиентуНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьСМСКлиенту(Команда)
	ОтправитьСМСКлиентуНаСервере();
КонецПроцедуры


&НаСервере
Процедура ПолуитьДанныеСделкиНаСервере()
...
&НаКлиенте
Процедура ПолуитьДанныеСделки(Команда)
	
	//ПолуитьДанныеСделкиНаСервере();
...
&НаСервере
Процедура ПолучитьДанныеАнкетыНаСервере()
...
Процедура ПолучитьДанныеАнкеты(Команда)
	...
	ID_Анкеты = ТекущиеДанные.ApplicationID;
	Ошибки = "";
	ДанныеАнкеты = УправлениеИпотечнымиЗаявкамиДомКлик.ПолучитьДанныеАнкеты(Объект.ПараметрыПодключения, Объект.ID_Заявки, ID_Анкеты, Ошибки, Объект.ТестоваяЗаявка);
...
&НаСервере
Процедура ОтправитьПереотправитьСМСКлиентуНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры
...
Процедура ОтправитьПереотправитьСМСКлиенту(Команда)
	...
	//ОтправитьПереотправитьСМСКлиентуНаСервере();
	...
	ID_Анкеты = ТекущиеДанные.ApplicationID;
	Ошибки = "";
	СтруктураВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.ОтправитьПереотправитьСМСКлиенту(Объект.ПараметрыПодключения, Объект.ID_Заявки, ID_Анкеты, Ошибки, Объект.ТестоваяЗаявка);
...
&НаСервере
Процедура ПодписатьСогласиеКодомИзСМСНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ПодписатьСогласиеКодомИзСМС(Команда)
	//ПодписатьСогласиеКодомИзСМСНаСервере();
...
Процедура ПослеВводаКодаПодтверждения(КодПодтверждения, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КодПодтверждения = Неопределено Тогда		
		...
		ID_Анкеты = ТекущиеДанные.ApplicationID;
		Ошибки = "";
		СтруктураВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.ПодписатьСогласиеКодомИзСМС(Объект.ПараметрыПодключения, Объект.ID_Заявки, ID_Анкеты, КодПодтверждения, Ошибки, Объект.ТестоваяЗаявка);
...
Процедура УдалениеДокументов(Команда)
	...
	ID_Анкеты = ТекущиеДанные.ApplicationID;
	ID_Документа = ТекущиеДанные.id;
	Ошибки = "";
	КодВозврата = УправлениеИпотечнымиЗаявкамиДомКлик.УдалениеДокументов(Объект.ПараметрыПодключения, ID_Анкеты, ID_Документа, Ошибки, Объект.ТестоваяЗаявка);
